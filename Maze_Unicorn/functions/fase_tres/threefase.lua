function labirinto_fase_three_draw()

	--love.graphics.rectangle( modo, x, y, largura, altura )

	--1 (amarelo)
	--love.graphics.setColor(247, 212, 14)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill",  0,   0, 80, 650 )




	--2 (vermelho)
	--love.graphics.setColor(226, 4, 4)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 75,  0, 1200, 60 )



	--3 (azul)
	--love.graphics.setColor(44, 49, 186)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 655, 100, 80, 650 )           



	--4-- (verde)
	--love.graphics.setColor(84, 186, 44)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 150,  650, 1200, 80 )



	--5-- (rosa)
	--love.graphics.setColor(206, 4, 162)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 800, 40, 80, 565 )




	--6 (azul ciano)
	--love.graphics.setColor(64, 225, 247)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 270,  40, 80, 565)




	--7 (verde neon)
	--love.graphics.setColor(0, 255, 38)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 125,  100, 80, 650)

	-- 8 laranja
	--love.graphics.setColor(237, 95, 7)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 930,  100, 100, 650 )




	--9 (cinza)
	--love.graphics.setColor(108, 132, 112)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 400,  100, 80, 650 )



	-- 10 (roxo)
	--love.graphics.setColor(146, 66, 244)
	love.graphics.setColor(231, 177, 249)
	love.graphics.rectangle("fill", 529.5,  40, 80, 565 )

end
